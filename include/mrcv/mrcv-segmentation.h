...
namespace mrcv
{
    ...
    inline torch::nn::Conv2dOptions conv_options(int64_t inPanes, int64_t outPanes, int64_t kernelSize,
        int64_t stride = 1, int64_t padding = 0, int groups = 1, bool with_bias = true, int dilation = 1) {
        torch::nn::Conv2dOptions conv_options = torch::nn::Conv2dOptions(inPanes, outPanes, kernelSize);
    ...
    {
    public:
        SegmentationBlockImpl(int inChannels, int outChannels, int upSamplesNum = 0);
        torch::Tensor forward(torch::Tensor x);
    private:
        torch::nn::Sequential block{ nullptr };
