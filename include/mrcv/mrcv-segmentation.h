...
namespace mrcv
{
    ...
    inline torch::nn::UpsampleOptions upsample_options(std::vector<double> scaleSize, bool alignCorners = true) {
        torch::nn::UpsampleOptions upsample_options = torch::nn::UpsampleOptions();
        upsample_options.scale_factor(scaleSize);
        upsample_options.mode(torch::kBilinear).align_corners(alignCorners);
    ...
    inline torch::nn::MaxPool2dOptions maxpool_options(int kernelSize, int stride) {
