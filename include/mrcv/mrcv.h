...
namespace mrcv
{
	...
	//MRCV_EXPORT std::vector< std::string > findUrl(std::string text);
	//MRCV_EXPORT int saveFile(std::string nameFile, std::vector< std::string > arrUrl);
	//MRCV_EXPORT void downloadFoto(std::vector< std::string > arrUrl, std::string patch, std::string nameFile,int count );
    ...
    MRCV_EXPORT int getImagesFromYandex(std::string patchBody, int count, int minwidth, int minheight, std::string nametemplate, std::string outputfolder, bool separatedataset = true, int trainsetpercentage = 70);
