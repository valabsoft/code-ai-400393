...
namespace mrcv
{
	...
	std::vector<int> findSubstring(std::string text, std::string word)
	{
	    ...
	    for (unsigned i {}; i <= text.length() - word.length(); )
        {
            ...
            substringAllPositions.push_back(substringPosition);
...
//основная функция
int getImagesFromYandex(std::string zapros,  int minWidth, int minHeight, std::string nameTemplate,  std::string outputFolder, bool separateDataset, unsigned int trainsetPercentage, unsigned int countFoto, bool money, std::string key, std::string secretKey)
{
