...
namespace mrcv
{
   void Predictor::trainLSTMNet(const std::vector<std::pair<float, float>> coordinates, bool isTraining) 
   {
    ...
    void PredictorCuda::trainLSTMNet(const std::vector<std::pair<float, float>> coordinates, bool isTraining) {
        std::vector<std::pair<float, float>> coordinatesNormalized = normilizeInput(coordinates);
        if (!coordinatesNormalized.empty()) {
            trainingData.clear();
            for (const auto& coord : coordinatesNormalized) {
    ...
    void PredictorCuda::continueTraining(const std::pair<float, float> coordinate) {
        coordsReal = coordinate;
        updateDeviations();
        std::pair<float, float> coordinate_norm = normilizePair(coordsReal);
    ...
    std::pair<float, float> PredictorCuda::predictNextCoordinate() {
        lstm->eval();
        linear->eval();
    ...
    void PredictorCuda::updateDeviations() {
        static float movingAvgPredDevSum = 0;
        static float predDevSum = 0;
        static int successedPredictions = 0;
    ...
    float PredictorCuda::getMovingAverageDeviation() {
        return movingAvgPredictionDeviation;
    }

    float PredictorCuda::getAverageDeviation() {
        return averagePredictionDeviation;
    }

    float PredictorCuda::getLastDeviation() {
        return predictionDeviation;
    }

    bool PredictorCuda::isWorkState() {
        return workState;
    }

    std::pair<float, float> PredictorCuda::normilizePair(std::pair<float, float> coords) {
        return std::make_pair(coords.first / imgWidth * 2 - 1, coords.second / imgHeight * 2 - 1);
    }

    std::vector<std::pair<float, float>> PredictorCuda::normilizeInput(std::vector<std::pair<float, float>> coords) {
        std::vector<std::pair<float, float>> coords_norm;
    ...
    std::pair<float, float> PredictorCuda::denormilizeOutput(std::pair<float, float> coords) {
        return { (coords.first + 1) * imgWidth / 2, (coords.second + 1) * imgHeight / 2 };
    }
#endif
