...
namespace mrcv
{
        ...
        std::vector<std::string>& listImages, std::vector<std::string>& listLabels)
    {
        for (auto& p : std::filesystem::directory_iterator(folder))
        {
            if (std::filesystem::path(p).extension() == imageType)
            {
                ...
                listLabels.push_back(replaceAllExtension(p.path().string(), imageType, ".json"));
            }
        }
        if (listImages.empty()) {
            writeLog("No images found in folder: " + folder);
        }
    }
    ...
    FPNImpl::FPNImpl(int _numberClasses, std::string encoderName, std::string pretrainedPath, int encoderDepth,
        ...
        if (!encoderParameter.contains(encoderName)) {
            writeLog("Unknown encoder: " + encoderName);
            throw std::runtime_error("Unknown encoder: " + encoderName);
        }
        ...
        if (encoderParameter[encoderName]["class_type"] == "resnet")
            ...
            writeLog("Unknown error in backbone initialization for encoder: " + encoderName);
            throw std::runtime_error("Unknown backbone type");
        }
        ...
        try {
            ...
            writeLog("Failed to load pretrained weights: " + pretrainedPath + ", Error: " + e.what());
            throw;
        }
    ...
    Conv3x3GNReLUImpl::Conv3x3GNReLUImpl(int _inChannels, int _outChannels, bool _upsample) {
        ...
        block = torch::nn::Sequential(
            torch::nn::Conv2d(conv_options(_inChannels, _outChannels, 3, 1, 1, 1, false)),
    ...
    torch::Tensor FPNBlockImpl::forward(torch::Tensor x, torch::Tensor skip) {
        ...
        skip = skip_conv->forward(skip);
        x = x + skip;
        return x;
    }

    SegmentationBlockImpl::SegmentationBlockImpl(int inChannels, int outChannels, int upSamplesNum)
    {
        block = torch::nn::Sequential();
        block->push_back(Conv3x3GNReLU(inChannels, outChannels, bool(upSamplesNum)));
        if (upSamplesNum > 1) {
            for (int i = 1; i < upSamplesNum; i++) {
    ...
    MergeBlockImpl::MergeBlockImpl(std::string policy) {
        if (policy != policies[0] && policy != policies[1]) {
            writeLog("Policy must be 'add' or 'cat', got: " + policy);
            throw std::runtime_error("Invalid merge policy");
        }
        _policy = policy;
    }
    ...
    torch::Tensor MergeBlockImpl::forward(std::vector<torch::Tensor> x) {
        if (_policy == "add") return sumTensor(x);
            ...
            writeLog("Policy must be 'add' or 'cat', got: " + _policy);
            throw std::runtime_error("Invalid merge policy");
        }
    }
        ...
        float dropout_, std::string merge_policy)
    {
        ...
        if (encoderDepth < 3) {
            writeLog("Encoder depth for FPN decoder cannot be less than 3");
            throw std::runtime_error("Invalid encoder depth");
        }
    ...
    torch::Tensor FPNDecoderImpl::forward(std::vector<torch::Tensor> features) {
        ...
        if (lengthFeatures < 4) {
            writeLog("Expected at least 4 feature maps, got: " + std::to_string(lengthFeatures));
            throw std::runtime_error("Insufficient feature maps");
        }
    ...
    torch::Tensor ResNetImpl::features_at(torch::Tensor x, int numStage) {
        if (numStage < 1 || numStage > 5) {
            writeLog("Stage number must be in range [1, 5], got: " + std::to_string(numStage));
            throw std::runtime_error("Invalid stage number");
        }
        x = conv1->forward(x);
        x = bn1->forward(x);
        x = torch::relu(x);
    ...
    void ResNetImpl::make_dilated(std::vector<int> listStage, std::vector<int> listDilation) {
        if (listStage.size() != listDilation.size()) {
            writeLog("Stage list length must equal dilation list length");
            throw std::runtime_error("Invalid dilation parameters");
        }
    ...
    ResNet resnet18(int64_t numberClasses) {
        ...
        return ResNet(layers, numberClasses, "resnet18");
    }

    ResNet resnet34(int64_t numberClasses) {
        std::vector<int> layers = { 3, 4, 6, 3 };
        return ResNet(layers, numberClasses, "resnet34");
    }

    ResNet resnet50(int64_t numberClasses) {
        std::vector<int> layers = { 3, 4, 6, 3 };
        return ResNet(layers, numberClasses, "resnet50");
    }
    ...
    ResNet pretrained_resnet(int64_t numberClasses, std::string nameModel, std::string pathWeight) {
        ...
        try {
            ...
            writeLog("Failed to load pretrained weights: " + pathWeight + ", Error: " + e.what());
            throw;
        }
    ...
    void Segmentor::InitializeCuda(int gpu_id, int _width, int _height, std::vector<std::string>&& _listName,
        ...
        if (gpu_id >= gpu_num) {
            writeLog("GPU ID exceeds max number of GPUs: " + std::to_string(gpu_id) + " vs " + std::to_string(gpu_num));
    ...
    void Segmentor::Predict(cv::Mat& image, const std::string& which_class) {
        try {
            ...
            tensor_image = tensor_image.to(torch::kFloat).div(255.0);

            torch::Tensor output = fpn->forward(tensor_image);
            output = torch::softmax(output, 1).mul(255.0).toType(torch::kByte);

            image = cv::Mat::ones(cv::Size(width, height), CV_8UC1);
            torch::Tensor re = output[0][which_class_index].to(at::kCPU).detach();
    ...
    void SegDataset::draw_mask(std::string pathJson, cv::Mat& mask) {
        ...
        if (!jfile.is_open()) {
            writeLog("Failed to open JSON: " + pathJson);
        ...
        try {
            ...
            writeLog("Invalid JSON format in: " + pathJson + ", Error: " + e.what());
            throw;
        }
        if (!j.contains("shapes")) {
            writeLog("JSON does not contain 'shapes' field: " + pathJson);
            throw std::runtime_error("Invalid JSON structure");
        }
        size_t num_blobs = j["shapes"].size();
        writeLog("Number of objects in " + pathJson + ": " + std::to_string(num_blobs));
        for (int i = 0; i < num_blobs; i++) {
            if (!j["shapes"][i].contains("label") || !j["shapes"][i].contains("points")) {
                writeLog("Missing 'label' or 'points' in shape " + std::to_string(i) + " of " + pathJson);
                continue;
            }
            std::string name = j["shapes"][i]["label"];
            if (name2index.find(name) == name2index.end()) {
                writeLog("Unknown class: " + name + " in " + pathJson);
                continue;
            }
            size_t points_len = j["shapes"][i]["points"].size();
            std::vector<cv::Point> contour;
            for (int t = 0; t < points_len; t++) {
                if (!j["shapes"][i]["points"][t].is_array() || j["shapes"][i]["points"][t].size() != 2) {
                    writeLog("Invalid point format in shape " + std::to_string(i) + " of " + pathJson);
                    continue;
                }
        ...
        trainTricks tricks, bool isTrain)
    {
        ...
        if (listName.size() > listColor.size()) {
            writeLog("Number of classes exceeds available colors: " + std::to_string(listName.size()));
            throw std::runtime_error("Too many classes for color list");
        }
        for (int i = 0; i < listName.size(); i++) {
    ...
    torch::data::Example<> SegDataset::get(size_t index) {
        try {
            ...
            if (image.empty()) {
                writeLog("Failed to load image: " + image_path);
            ...
            for (int i = 0; i < listName.size(); i++) {
                ...
                label_tensor = label_tensor + torch::all(colorful_label_tensor == color_tensor, -1).to(torch::kLong) * i;
            }
            label_tensor = label_tensor.unsqueeze(0);
            return { img_tensor.clone(), label_tensor.clone() };
        }
        catch (const std::exception& e) {
            writeLog("Error in SegDataset::get at index " + std::to_string(index) + ": " + e.what());
            throw;
        }
    }
}