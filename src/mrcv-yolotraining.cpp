...
{
	//inputΪbs, 3 * (5 + num_classes), 13, 13
	//һ��������ͼƬ
	...
	//���㲽����ÿһ���������Ӧԭ����ͼƬ�϶��ٸ����ص�
	//���������Ϊ13x13�Ļ���ԭͼ416x416������£�һ��������Ͷ�Ӧԭ����ͼƬ�ϵ�32�����ص�
...
std::vector<torch::Tensor> YOLOLossImpl::get_target(std::vector<torch::Tensor> targets, torch::Tensor scaled_anchors, int in_w, int in_h, float ignore_threshold)
{
	...
	int subtract_index = 3 * index;//0����3����6
	//����ȫ��0����ȫ��1������
	...
	for (int b = 0; b < bs; b++)
	{
		if (targets[b].sizes().size() == 1)//dataset����targets��ͼƬ��box��tensor = torch::ones({1});
		{
			continue;
		}
		//��������������ϵĵ�λ
...
std::vector<int> nms_libtorch(torch::Tensor bboxes, torch::Tensor scores, float thresh) {
	...
	auto areas = (x2 - x1) * (y2 - y1);   //[N, ] ÿ��bbox�����
	auto tuple_sorted = scores.sort(0, true);    //��������
	auto order = std::get<1>(tuple_sorted);

	std::vector<int>	keep;
	while (order.numel() > 0) {// torch.numel()��������Ԫ�ظ���
		if (order.numel() == 1) {//    ������ֻʣһ��
			auto i = order.item();
			keep.push_back(i.toInt());
			break;
		}
		else {
			auto i = order[0].item();// ����scores�����Ǹ���box[i]
			keep.push_back(i.toInt());
		}
		//����box[i]����������IOU(˼·�ܺ�)
		...
		auto idx = (iou <= thresh).nonzero().squeeze();//ע���ʱidxΪ[N - 1, ] ��orderΪ[N, ]
		if (idx.numel() == 0) {
			break;
		}
		order = order.index({ idx + 1 }); //�޲�����֮��Ĳ�ֵ
	}
	return keep;
}
