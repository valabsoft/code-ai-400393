...
namespace mrcv
{
	...
	std::vector< std::string > urlFind(std::string text) {
		int ind=0;
	...
	void downloadFoto(std::vector< std::string > arrUrl, std::string patch)
	{
		for (unsigned long i=0; i<arrUrl.size()-1;i++ )
		{
            ...
            std::string concat_str = str1 + patch + str3; 
